...
public class ElevatorSimulation extends SimulationBase {
    ...
    public ElevatorSimulation(ElevatorSimulationConfig config) {
        this.config = config;
        this.elevatorSim = new ElevatorSim(config.leader, config.gearing, config.carriageMassKg, config.drumRadiusMeters, config.minHeightMeters, config.maxHeightMeters, true, config.minHeightMeters, null);

        this.mech2d = new Mechanism2d(3.0, config.maxHeightMeters + 1.0);
        this.root = mech2d.getRoot("ElevatorRoot", 1.5, config.minHeightMeters);
        this.elevatorLigament = root.append(new MechanismLigament2d("Elevator", config.minHeightMeters, 90));
    }
