...
public class Arm extends AbsoluteSubsystem {
	...
	public static class Joint {
		...
		public Joint(MotorWrapper motor, MotorConfig motorConfig, EncoderWrapper encoder, JointConfig config, double initialAngleRad) {
			...
			this.config = config;
			if (motorConfig != null) motor.applyMotorConfig(motorConfig);
		...
		private double clampPercent(double p){ return DoubleUtils.clamp(p, minOutputPercent, maxOutputPercent); }
		public void setManualPercent(double pct){ manualPercent = clampPercent(pct); setMode(Mode.MANUAL);}        
		public void setTargetAngleRadians(double radians){ targetRadians = DoubleUtils.clamp(radians,config.minAngleRad,config.maxAngleRad); setMode(Mode.POSITION); positionPid.reset(); }        
		public void stop(){ manualPercent=0; applyPercentOutput(0); setMode(Mode.IDLE);}        
		// Optimized: return cached angle updated in periodic
		public double getAngleRadians(){ return cachedAngleRad; }
		private void updateCachedAngle(){ cachedAngleRad = encoder.getPositionMeters()*config.metersToRadians; }
		public double getTargetAngleRadians(){ return targetRadians; }
		public boolean atTarget(){ return Math.abs(cachedAngleRad-targetRadians) <= config.toleranceRad; }
		public Mode getMode(){ return mode; }
		public void setEncoder(EncoderWrapper e){ this.encoder = e; }

		void periodic(){
			updateCachedAngle(); // read encoder once per cycle
			onPrePeriodic();
			// Optimized: compute output once
		...
		protected double computeOutputPercent(Mode mode, double target, double current){
			...
			switch(mode){
				...
				case POSITION: {
					...
					base = ffPercent + pidPercent; break; }
				case HOLDING: {
		...
		protected double applyAugmentors(double base, double target, double current){
			...
			for (var fn : augmentors){
				try { out += fn.apply(target, current); } catch(Exception ignored){}
			}
			return out;
		}
		...
		public void setFeedforwardGains(double kS,double kCos,double kV,double kA){ feedforward = new ArmFeedforward(kS,kCos,kV,kA); }
		public void setDesiredVelocity(double velocityRadPerSec){ desiredVelocityRadPerSec = velocityRadPerSec; }
		public void setNominalVoltage(double volts){ nominalVoltage = volts; }
		public void addOutputAugmentor(java.util.function.BiFunction<Double,Double,Double> augment){ if(augment!=null) augmentors.add(augment);}    
		public boolean removeOutputAugmentor(java.util.function.BiFunction<Double,Double,Double> augment){ return augmentors.remove(augment);}    
		public void clearOutputAugmentors(){ augmentors.clear(); }
		...
		protected double afterBaseCompute(Mode mode, double targetRad, double currentRad, double basePercent){ return basePercent; }
		protected double afterAugmentCompute(Mode mode, double targetRad, double currentRad, double percent){ return percent; }
		...
		private void setMode(Mode newMode){ if(mode!=newMode){ Mode prev=mode; mode=newMode; onModeChanged(prev,newMode);} }

		@FunctionalInterface
		public interface OutputFilter { double filter(double targetRad, double currentRad, double proposedPercent); }
		private final java.util.List<OutputFilter> outputFilters = new java.util.ArrayList<>();
		public void addOutputFilter(OutputFilter f){ if(f!=null) outputFilters.add(f);} 
		public boolean removeOutputFilter(OutputFilter f){ return outputFilters.remove(f);} 
		...
		protected double applyOutputFilters(double targetRad, double currentRad, double proposedPercent){
			...
			for(OutputFilter f: outputFilters){ try { p = f.filter(targetRad, currentRad, p);} catch(Exception ignored){} }
			return p;
		}
	}
	...
	public static class JointConfig {
		...
		public static class Builder { 
			...
			public Builder minAngleRad(double v){minAngleRad=v;return this;} 
			public Builder maxAngleRad(double v){maxAngleRad=v;return this;} 
			public Builder toleranceRad(double v){toleranceRad=v;return this;} 
			public Builder metersToRadians(double v){metersToRadians=v;return this;} 
			public Builder linkLengthMeters(double v){linkLengthMeters=v;return this;}
			public JointConfig build(){ return new JointConfig(this);} 
		}
	}
	...
	public void setTargetAngles(double... radians){ 
		...
		for(int i=0;i<n;i++) joints.get(i).setTargetAngleRadians(radians[i]); 
	}
	public void setManualPercents(double... percents){ 
		ikMode = false;
		int n=Math.min(percents.length,joints.size()); 
		for(int i=0;i<n;i++) joints.get(i).setManualPercent(percents[i]); 
	}
	...
	public boolean isIKMode(){ return ikMode; }
	public boolean isIKSolved(){ return ikSolved; }
	public Translation2d getGoalPose(){ return goalPose; }
	...
	public boolean allAtTargets(){ 
		for(Joint j: joints) if(!j.atTarget()) return false; 
		return true; 
	}
	...
	// Forward kinematics helper (for debugging/telemetry)
	public Translation2d getEndEffectorPose(){
		if(joints.isEmpty()) return new Translation2d();
		double[] angles = new double[joints.size()];
		for(int i=0;i<joints.size();i++) angles[i] = joints.get(i).getAngleRadians();
		return computeEndEffector(angles);
	}
